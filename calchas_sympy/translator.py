from calchas_datamodel import AbstractVisitor, AbstractExpression, Placeholder, IdExpression, FunctionCallExpression, \
    FormulaFunctionExpression, IntegerLiteralCalchasExpression, FloatLiteralCalchasExpression, ConstantValueExpression, \
    ConstantFunctionExpression, GenericConstantFunction, ConstantFunctionCallExpression, \
...
class CalchasTreeBuilder:
    def __init__(self,
                 ...
                 context: Optional[Tuple[Dict[str, sympy.Expr], Dict[str, AbstractSympyFunction]]] = None):
        self.tree = tree
...
class SympyTreeBuilder(AbstractVisitor):
    def __init__(self,
                 tree: AbstractExpression,
                 context: Optional[Tuple[Dict[ConstantValueExpression, sympy.Expr],
                                         Dict[ConstantFunctionExpression, AbstractSympyFunction]]] = None):
        super().__init__(tree)
        self.variables = context[0] if context is None else base_constants
        self.functions = context[1] if context is None else base_functions
